...
const MOCK_DATA = {
  ...
  topProducts: [
    { name: 'Burger Vegen', qty: 84, type: 'STAR' },
    { name: 'Nexus BBQ', qty: 62, type: 'PLOW' },
    { name: 'Cerveza Artesana', qty: 15, type: 'PUZZLE' },
    { name: 'Postre de Soja', qty: 5, type: 'DOG' }
  ],
  anomalies: [ { title: "Sobremesa crítica", desc: "Mesa 14 (+45m)" } ]
};

function App() {
  const [view, setView] = useState('dashboard'); // 'dashboard' | 'sales' | 'products'
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] p-4 md:p-8 max-w-7xl mx-auto space-y-8 pb-20">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tighter">VEGEN <span className="text-green-500 font-light">MANAGER LITE</span></h1>
          <p className="text-gray-500 text-[9px] uppercase tracking-[0.3em]">HORECA Intelligence</p>
        </div>
        
        {/* Selector de Turnos (Mejora #3) */}
        <div className="hidden md:flex p-1 bg-gray-900 rounded-xl border border-gray-800">
          {['MAÑANA', 'MEDIODÍA', 'NOCHE'].map(t => (
            <button key={t} onClick={() => setActiveTurn(t)} 
              className={`px-3 py-1.5 rounded-lg text-[9px] font-black transition-all ${activeTurn === t ? 'bg-green-500 text-black' : 'text-gray-500'}`}>
              {t}
            </button>
          ))}
        </div>
