    ...
    <div onClick={onClick} className="bg-[#161616] p-6 rounded-3xl border border-gray-800 hover:border-green-500/50 transition-all cursor-pointer group relative overflow-hidden">
      <div className="flex justify-between items-start relative z-10">
        ...
        <div className="flex flex-col items-end">
          <span className={`text-[9px] font-black px-2 py-1 rounded-md ${isPositive ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>{trend}</span>
          <svg className="w-16 h-8 mt-2 opacity-50 group-hover:opacity-100 transition-opacity" viewBox="0 0 100 40">
            <path d={isPositive ? "M0 35 L20 30 L40 32 L60 15 L80 20 L100 5" : "M0 5 L20 15 L40 10 L60 30 L80 25 L100 38"} 
                  fill="none" stroke={isPositive ? "#22C55E" : "#EF4444"} strokeWidth="4" strokeLinecap="round" />
          </svg>
        </div>
      </div>
